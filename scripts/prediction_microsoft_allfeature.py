"""
Microsoft Stock Price Prediction using LSTM with All Features
This script predicts Microsoft stock prices using LSTM neural networks
with all available features (Volume, Open, High, Low).
"""
...
def load_and_preprocess_data(file_path):
    ...
    try:
        ...
        return volume, open_price, high, low, target_var, features
        
    except Exception as e:
        print(f"Error loading data: {e}")
        return None, None, None, None, None, None

def scale_features(volume, open_price, high, low, target_var):
    ...
    # Combine features
    features_combined = np.column_stack([volume_scaled, open_scaled, high_scaled, low_scaled])
...
def plot_results(y_test, y_pred):
    ...
    plt.title('Microsoft Stock Price Prediction - LSTM (All Features)')
...
def main():
    ...
    print("Microsoft Stock Price Prediction using LSTM")
    print("=" * 50)
    ...
    volume, open_price, high, low, target_var, features = load_and_preprocess_data(data_path)
    
    if volume is None:
        return
    
    # Scale features
    features_scaled, target_scaled, scaler = scale_features(volume, open_price, high, low, target_var)
    ...
    # Save model
    model.save('microsoft_lstm_allfeatures.h5')
    print("\nModel saved as 'microsoft_lstm_allfeatures.h5'")
